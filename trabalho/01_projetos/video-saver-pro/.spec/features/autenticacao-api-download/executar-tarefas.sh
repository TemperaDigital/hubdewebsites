#!/usr/bin/env bash
# executar-tarefas.sh — gerado por `onp-spec plano autenticacao-api-download` em 2026-08-31 06:25
...
#
# resumo do que está rolando, a qualquer momento: onp-spec resumo autenticacao-api-download
set -u
set -o pipefail

RUN_ID='video-saver-pro-autenticacao-api-download-mtgutnon'
FEATURE='autenticacao-api-download'
BASE_BRANCH='spec/autenticacao-api-download'
...
preparar_ambiente() {
  ...
  git ls-files --error-unmatch -- '.spec/features/autenticacao-api-download/spec.md' >/dev/null 2>&1 || falhar "spec.md não está commitada — os worktrees das faixas precisam dela no git"
  ...
  git worktree prune
  LOG_DIR="$(dirname "$TOPLEVEL")/onp-worktrees/video-saver-pro-autenticacao-api-download-logs"
  WT_BASE="$(dirname "$TOPLEVEL")/onp-worktrees/video-saver-pro-autenticacao-api-download"
...
mesclar_faixa() { # $1=faixa $2=branch $3=worktree $4=exit-da-faixa
  if [ "$4" -ne 0 ]; then
    ...
    amarelo "  reexecute só ela: bash .spec/features/autenticacao-api-download/executar-tarefas.sh --faixa $1"
...
executar_faixa_1() {
  ...
  preparar_worktree 'faixa-1' 'spec/autenticacao-api-download-faixa-1' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-1' --estado executando --tentativa "$(tentativa 'faixa-1')"
  : > "$LOG_DIR/faixa-1.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-1' 'T-010' 'Você executa UMA tarefa da feature "autenticacao-api-download" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/autenticacao-api-download/spec.md, .spec/features/autenticacao-api-download/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-010 — "Conferir credencial em tempo constante"
  critérios/refs: AC-009 (Pedido sem credencial é recusado), AC-010 (Pedido com a credencial certa funciona igual), AC-011 (Sem senha configurada, o comportamento de hoje é preservado)
  arquivos permitidos (e seus testes): extractor/auth.js, extractor/auth.test.js
  mensagem de commit: "T-010 autenticacao-api-download: Conferir credencial em tempo constante"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-1' 'spec/autenticacao-api-download-faixa-1' "$WT" "$st" || return 1
  marcar_concluidas T-010
  return 0
}

# ── faixa-2: T-011 T-012 T-014 T-015 ──
executar_faixa_2() {
  local WT="$WT_BASE-faixa-2"
  preparar_worktree 'faixa-2' 'spec/autenticacao-api-download-faixa-2' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-2' --estado executando --tentativa "$(tentativa 'faixa-2')"
  : > "$LOG_DIR/faixa-2.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-2' 'T-011' 'Você executa UMA tarefa da feature "autenticacao-api-download" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/autenticacao-api-download/spec.md, .spec/features/autenticacao-api-download/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-011 — "Ligar a verificação nas rotas do motor"
  critérios/refs: AC-009 (Pedido sem credencial é recusado), AC-010 (Pedido com a credencial certa funciona igual), AC-011 (Sem senha configurada, o comportamento de hoje é preservado)
  arquivos permitidos (e seus testes): extractor/server.js
  mensagem de commit: "T-011 autenticacao-api-download: Ligar a verificação nas rotas do motor"
...
- NÃO edite tasks.md, NÃO rode onp-spec verify/audit e NÃO toque em outras tarefas — o orquestrador cuida disso.
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium &&
    rodar_tarefa 'faixa-2' 'T-012' 'Você executa UMA tarefa da feature "autenticacao-api-download" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/autenticacao-api-download/spec.md, .spec/features/autenticacao-api-download/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-012 — "Fechar o CORS aberto"
  critérios/refs: AC-012 (Origem desconhecida não é aceita)
  arquivos permitidos (e seus testes): extractor/server.js, docker-compose.yml
  mensagem de commit: "T-012 autenticacao-api-download: Fechar o CORS aberto"
...
- NÃO edite tasks.md, NÃO rode onp-spec verify/audit e NÃO toque em outras tarefas — o orquestrador cuida disso.
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium &&
    rodar_tarefa 'faixa-2' 'T-014' 'Você executa UMA tarefa da feature "autenticacao-api-download" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/autenticacao-api-download/spec.md, .spec/features/autenticacao-api-download/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-014 — "Aplicar o limite na rota de download"
  critérios/refs: AC-013 (Passou do limite, entra na fila em vez de derrubar a máquina)
  arquivos permitidos (e seus testes): extractor/server.js
  mensagem de commit: "T-014 autenticacao-api-download: Aplicar o limite na rota de download"
...
- NÃO edite tasks.md, NÃO rode onp-spec verify/audit e NÃO toque em outras tarefas — o orquestrador cuida disso.
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium &&
    rodar_tarefa 'faixa-2' 'T-015' 'Você executa UMA tarefa da feature "autenticacao-api-download" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/autenticacao-api-download/spec.md, .spec/features/autenticacao-api-download/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-015 — "Atualizar a documentação de instalação"
  critérios/refs: AC-011 (Sem senha configurada, o comportamento de hoje é preservado)
  arquivos permitidos (e seus testes): README-docker.md, docker-compose.yml
  mensagem de commit: "T-015 autenticacao-api-download: Atualizar a documentação de instalação"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-2' 'spec/autenticacao-api-download-faixa-2' "$WT" "$st" || return 1
  marcar_concluidas T-011 T-012 T-014 T-015
  return 0
}
...
executar_faixa_3() {
  ...
  preparar_worktree 'faixa-3' 'spec/autenticacao-api-download-faixa-3' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-3' --estado executando --tentativa "$(tentativa 'faixa-3')"
  : > "$LOG_DIR/faixa-3.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-3' 'T-013' 'Você executa UMA tarefa da feature "autenticacao-api-download" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/autenticacao-api-download/spec.md, .spec/features/autenticacao-api-download/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-013 — "Limitar downloads simultâneos"
  critérios/refs: AC-013 (Passou do limite, entra na fila em vez de derrubar a máquina)
  arquivos permitidos (e seus testes): extractor/concurrency.js, extractor/concurrency.test.js
  mensagem de commit: "T-013 autenticacao-api-download: Limitar downloads simultâneos"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-3' 'spec/autenticacao-api-download-faixa-3' "$WT" "$st" || return 1
...
encerrar() { # $1=escopo
  ...
  if [ "$COM_GATE" -eq 0 ]; then
    ...
    if [ -z "$FALHAS" ]; then
      ...
      amarelo "  para o veredito: bash .spec/features/autenticacao-api-download/executar-tarefas.sh --gate"
      exit 0
    fi
...
listar() {
  ...
  echo "  faixa-2  onda 1  T-011, T-012, T-014, T-015"
  echo "  faixa-3  onda 1  T-013"
  echo
