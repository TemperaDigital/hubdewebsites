#!/usr/bin/env bash
# executar-tarefas.sh — gerado por `onp-spec plano corrigir-merge-video-audio` em 2026-08-31 06:25
...
#
# resumo do que está rolando, a qualquer momento: onp-spec resumo corrigir-merge-video-audio
set -u
set -o pipefail

RUN_ID='video-saver-pro-corrigir-merge-video-audio-mtgutm9a'
FEATURE='corrigir-merge-video-audio'
BASE_BRANCH='spec/corrigir-merge-video-audio'
...
preparar_ambiente() {
  ...
  git ls-files --error-unmatch -- '.spec/features/corrigir-merge-video-audio/spec.md' >/dev/null 2>&1 || falhar "spec.md não está commitada — os worktrees das faixas precisam dela no git"
  ...
  git worktree prune
  LOG_DIR="$(dirname "$TOPLEVEL")/onp-worktrees/video-saver-pro-corrigir-merge-video-audio-logs"
  WT_BASE="$(dirname "$TOPLEVEL")/onp-worktrees/video-saver-pro-corrigir-merge-video-audio"
...
mesclar_faixa() { # $1=faixa $2=branch $3=worktree $4=exit-da-faixa
  if [ "$4" -ne 0 ]; then
    ...
    amarelo "  reexecute só ela: bash .spec/features/corrigir-merge-video-audio/executar-tarefas.sh --faixa $1"
...
executar_faixa_1() {
  ...
  preparar_worktree 'faixa-1' 'spec/corrigir-merge-video-audio-faixa-1' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-1' --estado executando --tentativa "$(tentativa 'faixa-1')"
  : > "$LOG_DIR/faixa-1.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-1' 'T-001' 'Você executa UMA tarefa da feature "corrigir-merge-video-audio" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/corrigir-merge-video-audio/spec.md, .spec/features/corrigir-merge-video-audio/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-001 — "Detectar o container real de um arquivo pelos bytes de assinatura"
  critérios/refs: AC-001 (O arquivo entregue é do formato que a extensão promete)
  arquivos permitidos (e seus testes): src/lib/container-signature.ts, src/lib/container-signature.test.ts
  mensagem de commit: "T-001 corrigir-merge-video-audio: Detectar o container real de um arquivo pelos bytes de assinatura"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-1' 'spec/corrigir-merge-video-audio-faixa-1' "$WT" "$st" || return 1
...
executar_faixa_2() {
  ...
  preparar_worktree 'faixa-2' 'spec/corrigir-merge-video-audio-faixa-2' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-2' --estado executando --tentativa "$(tentativa 'faixa-2')"
  : > "$LOG_DIR/faixa-2.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-2' 'T-002' 'Você executa UMA tarefa da feature "corrigir-merge-video-audio" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/corrigir-merge-video-audio/spec.md, .spec/features/corrigir-merge-video-audio/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-002 — "Provar o defeito com teste de regressão sobre saída real do yt-dlp"
  critérios/refs: AC-001 (O arquivo entregue é do formato que a extensão promete), AC-002 (O vídeo baixado tem imagem e som), AC-003 (A qualidade escolhida na tela é a qualidade entregue)
  arquivos permitidos (e seus testes): extractor/fetch-args.js, extractor/fetch-args.test.js
  mensagem de commit: "T-002 corrigir-merge-video-audio: Provar o defeito com teste de regressão sobre saída real do yt-dlp"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-2' 'spec/corrigir-merge-video-audio-faixa-2' "$WT" "$st" || return 1
  marcar_concluidas T-002
  return 0
}

# ── faixa-3: T-003 T-004 T-005 ──
executar_faixa_3() {
  local WT="$WT_BASE-faixa-3"
  preparar_worktree 'faixa-3' 'spec/corrigir-merge-video-audio-faixa-3' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-3' --estado executando --tentativa "$(tentativa 'faixa-3')"
  : > "$LOG_DIR/faixa-3.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-3' 'T-003' 'Você executa UMA tarefa da feature "corrigir-merge-video-audio" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/corrigir-merge-video-audio/spec.md, .spec/features/corrigir-merge-video-audio/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-003 — "Implementar a estratégia de container escolhida"
  critérios/refs: AC-001 (O arquivo entregue é do formato que a extensão promete), AC-002 (O vídeo baixado tem imagem e som), AC-003 (A qualidade escolhida na tela é a qualidade entregue)
  arquivos permitidos (e seus testes): extractor/server.js
  mensagem de commit: "T-003 corrigir-merge-video-audio: Implementar a estratégia de container escolhida"
...
- NÃO edite tasks.md, NÃO rode onp-spec verify/audit e NÃO toque em outras tarefas — o orquestrador cuida disso.
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium &&
    rodar_tarefa 'faixa-3' 'T-004' 'Você executa UMA tarefa da feature "corrigir-merge-video-audio" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/corrigir-merge-video-audio/spec.md, .spec/features/corrigir-merge-video-audio/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-004 — "Alinhar extensão e Content-Type ao container que sai de fato"
  critérios/refs: AC-001 (O arquivo entregue é do formato que a extensão promete), AC-004 (Container trocado vira falha, não sucesso)
  arquivos permitidos (e seus testes): extractor/server.js, src/lib/media-format.ts
  mensagem de commit: "T-004 corrigir-merge-video-audio: Alinhar extensão e Content-Type ao container que sai de fato"
...
- NÃO edite tasks.md, NÃO rode onp-spec verify/audit e NÃO toque em outras tarefas — o orquestrador cuida disso.
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium &&
    rodar_tarefa 'faixa-3' 'T-005' 'Você executa UMA tarefa da feature "corrigir-merge-video-audio" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/corrigir-merge-video-audio/spec.md, .spec/features/corrigir-merge-video-audio/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-005 — "Transformar divergência de container em erro visível"
  critérios/refs: AC-004 (Container trocado vira falha, não sucesso)
  arquivos permitidos (e seus testes): extractor/server.js
  mensagem de commit: "T-005 corrigir-merge-video-audio: Transformar divergência de container em erro visível"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-3' 'spec/corrigir-merge-video-audio-faixa-3' "$WT" "$st" || return 1
  marcar_concluidas T-003 T-004 T-005
  return 0
}
...
encerrar() { # $1=escopo
  ...
  if [ "$COM_GATE" -eq 0 ]; then
    ...
    if [ -z "$FALHAS" ]; then
      ...
      amarelo "  para o veredito: bash .spec/features/corrigir-merge-video-audio/executar-tarefas.sh --gate"
      exit 0
    fi
...
listar() {
  ...
  echo "  faixa-3  onda 1  T-003, T-004, T-005"
  echo
