#!/usr/bin/env bash
# executar-tarefas.sh — gerado por `onp-spec plano download-sem-buffer-memoria` em 2026-08-31 06:25
...
#
# resumo do que está rolando, a qualquer momento: onp-spec resumo download-sem-buffer-memoria
set -u
set -o pipefail

RUN_ID='video-saver-pro-download-sem-buffer-memoria-mtgutmxt'
FEATURE='download-sem-buffer-memoria'
BASE_BRANCH='spec/download-sem-buffer-memoria'
...
preparar_ambiente() {
  ...
  git ls-files --error-unmatch -- '.spec/features/download-sem-buffer-memoria/spec.md' >/dev/null 2>&1 || falhar "spec.md não está commitada — os worktrees das faixas precisam dela no git"
  ...
  git worktree prune
  LOG_DIR="$(dirname "$TOPLEVEL")/onp-worktrees/video-saver-pro-download-sem-buffer-memoria-logs"
  WT_BASE="$(dirname "$TOPLEVEL")/onp-worktrees/video-saver-pro-download-sem-buffer-memoria"
...
mesclar_faixa() { # $1=faixa $2=branch $3=worktree $4=exit-da-faixa
  if [ "$4" -ne 0 ]; then
    ...
    amarelo "  reexecute só ela: bash .spec/features/download-sem-buffer-memoria/executar-tarefas.sh --faixa $1"
...
executar_faixa_1() {
  ...
  preparar_worktree 'faixa-1' 'spec/download-sem-buffer-memoria-faixa-1' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-1' --estado executando --tentativa "$(tentativa 'faixa-1')"
  : > "$LOG_DIR/faixa-1.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-1' 'T-006' 'Você executa UMA tarefa da feature "download-sem-buffer-memoria" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/download-sem-buffer-memoria/spec.md, .spec/features/download-sem-buffer-memoria/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-006 — "Detectar se o navegador consegue gravar direto no disco"
  critérios/refs: AC-006 (Navegador sem suporte continua funcionando)
  arquivos permitidos (e seus testes): src/lib/file-sink.ts, src/lib/file-sink.test.ts
  mensagem de commit: "T-006 download-sem-buffer-memoria: Detectar se o navegador consegue gravar direto no disco"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-1' 'spec/download-sem-buffer-memoria-faixa-1' "$WT" "$st" || return 1
  marcar_concluidas T-006
  return 0
}

# ── faixa-2: T-007 T-008 T-009 ──
executar_faixa_2() {
  local WT="$WT_BASE-faixa-2"
  preparar_worktree 'faixa-2' 'spec/download-sem-buffer-memoria-faixa-2' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-2' --estado executando --tentativa "$(tentativa 'faixa-2')"
  : > "$LOG_DIR/faixa-2.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-2' 'T-007' 'Você executa UMA tarefa da feature "download-sem-buffer-memoria" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/download-sem-buffer-memoria/spec.md, .spec/features/download-sem-buffer-memoria/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-007 — "Separar a leitura do stream da forma de salvar"
  critérios/refs: AC-005 (Vídeo grande vai para o disco sem acumular na memória), AC-008 (A barra de progresso continua funcionando no caminho novo)
  arquivos permitidos (e seus testes): src/lib/downloader-client.ts, src/lib/downloader-client.test.ts
  mensagem de commit: "T-007 download-sem-buffer-memoria: Separar a leitura do stream da forma de salvar"
...
- NÃO edite tasks.md, NÃO rode onp-spec verify/audit e NÃO toque em outras tarefas — o orquestrador cuida disso.
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium &&
    rodar_tarefa 'faixa-2' 'T-008' 'Você executa UMA tarefa da feature "download-sem-buffer-memoria" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/download-sem-buffer-memoria/spec.md, .spec/features/download-sem-buffer-memoria/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-008 — "Implementar a gravação direta em disco"
  critérios/refs: AC-005 (Vídeo grande vai para o disco sem acumular na memória), AC-008 (A barra de progresso continua funcionando no caminho novo)
  arquivos permitidos (e seus testes): src/lib/downloader-client.ts
  mensagem de commit: "T-008 download-sem-buffer-memoria: Implementar a gravação direta em disco"
...
- NÃO edite tasks.md, NÃO rode onp-spec verify/audit e NÃO toque em outras tarefas — o orquestrador cuida disso.
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium &&
    rodar_tarefa 'faixa-2' 'T-009' 'Você executa UMA tarefa da feature "download-sem-buffer-memoria" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/download-sem-buffer-memoria/spec.md, .spec/features/download-sem-buffer-memoria/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-009 — "Avisar em português quando o arquivo não couber na memória"
  critérios/refs: AC-007 (Estouro de memória vira aviso em português, não tela branca)
  arquivos permitidos (e seus testes): src/lib/downloader-client.ts, src/components/downloader/DownloadBar.tsx
  mensagem de commit: "T-009 download-sem-buffer-memoria: Avisar em português quando o arquivo não couber na memória"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-2' 'spec/download-sem-buffer-memoria-faixa-2' "$WT" "$st" || return 1
  marcar_concluidas T-007 T-008 T-009
  return 0
}
...
encerrar() { # $1=escopo
  ...
  if [ "$COM_GATE" -eq 0 ]; then
    ...
    if [ -z "$FALHAS" ]; then
      ...
      amarelo "  para o veredito: bash .spec/features/download-sem-buffer-memoria/executar-tarefas.sh --gate"
      exit 0
    fi
...
listar() {
  ...
  echo "  faixa-2  onda 1  T-007, T-008, T-009"
  echo
