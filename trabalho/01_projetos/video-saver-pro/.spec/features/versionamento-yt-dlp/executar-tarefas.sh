#!/usr/bin/env bash
# executar-tarefas.sh — gerado por `onp-spec plano versionamento-yt-dlp` em 2026-08-31 06:25
...
#
# resumo do que está rolando, a qualquer momento: onp-spec resumo versionamento-yt-dlp
set -u
set -o pipefail

RUN_ID='video-saver-pro-versionamento-yt-dlp-mtgutoav'
FEATURE='versionamento-yt-dlp'
BASE_BRANCH='spec/versionamento-yt-dlp'
...
preparar_ambiente() {
  ...
  git ls-files --error-unmatch -- '.spec/features/versionamento-yt-dlp/spec.md' >/dev/null 2>&1 || falhar "spec.md não está commitada — os worktrees das faixas precisam dela no git"
  ...
  git worktree prune
  LOG_DIR="$(dirname "$TOPLEVEL")/onp-worktrees/video-saver-pro-versionamento-yt-dlp-logs"
  WT_BASE="$(dirname "$TOPLEVEL")/onp-worktrees/video-saver-pro-versionamento-yt-dlp"
...
mesclar_faixa() { # $1=faixa $2=branch $3=worktree $4=exit-da-faixa
  if [ "$4" -ne 0 ]; then
    ...
    amarelo "  reexecute só ela: bash .spec/features/versionamento-yt-dlp/executar-tarefas.sh --faixa $1"
...
executar_faixa_1() {
  ...
  preparar_worktree 'faixa-1' 'spec/versionamento-yt-dlp-faixa-1' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-1' --estado executando --tentativa "$(tentativa 'faixa-1')"
  : > "$LOG_DIR/faixa-1.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-1' 'T-016' 'Você executa UMA tarefa da feature "versionamento-yt-dlp" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/versionamento-yt-dlp/spec.md, .spec/features/versionamento-yt-dlp/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-016 — "Ler versão e estado da atualização do yt-dlp"
  critérios/refs: AC-014 (A versão em uso é visível), AC-015 (Falha de atualização aparece, em vez de sumir)
  arquivos permitidos (e seus testes): extractor/ytdlp-info.js, extractor/ytdlp-info.test.js
  mensagem de commit: "T-016 versionamento-yt-dlp: Ler versão e estado da atualização do yt-dlp"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-1' 'spec/versionamento-yt-dlp-faixa-1' "$WT" "$st" || return 1
  marcar_concluidas T-016
  return 0
}

# ── faixa-2: T-017 T-018 T-019 T-020 T-021 ──
executar_faixa_2() {
  local WT="$WT_BASE-faixa-2"
  preparar_worktree 'faixa-2' 'spec/versionamento-yt-dlp-faixa-2' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-2' --estado executando --tentativa "$(tentativa 'faixa-2')"
  : > "$LOG_DIR/faixa-2.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-2' 'T-017' 'Você executa UMA tarefa da feature "versionamento-yt-dlp" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/versionamento-yt-dlp/spec.md, .spec/features/versionamento-yt-dlp/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-017 — "Mostrar versão e estado na saúde do serviço"
  critérios/refs: AC-014 (A versão em uso é visível), AC-015 (Falha de atualização aparece, em vez de sumir)
  arquivos permitidos (e seus testes): extractor/server.js
  mensagem de commit: "T-017 versionamento-yt-dlp: Mostrar versão e estado na saúde do serviço"
...
- NÃO edite tasks.md, NÃO rode onp-spec verify/audit e NÃO toque em outras tarefas — o orquestrador cuida disso.
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium &&
    rodar_tarefa 'faixa-2' 'T-018' 'Você executa UMA tarefa da feature "versionamento-yt-dlp" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/versionamento-yt-dlp/spec.md, .spec/features/versionamento-yt-dlp/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-018 — "Registrar a tentativa de atualização em vez de engolir a falha"
  critérios/refs: AC-015 (Falha de atualização aparece, em vez de sumir)
  arquivos permitidos (e seus testes): docker/Dockerfile.extractor
  mensagem de commit: "T-018 versionamento-yt-dlp: Registrar a tentativa de atualização em vez de engolir a falha"
...
- NÃO edite tasks.md, NÃO rode onp-spec verify/audit e NÃO toque em outras tarefas — o orquestrador cuida disso.
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium &&
    rodar_tarefa 'faixa-2' 'T-019' 'Você executa UMA tarefa da feature "versionamento-yt-dlp" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/versionamento-yt-dlp/spec.md, .spec/features/versionamento-yt-dlp/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-019 — "Ativar um runtime de JavaScript para a extração do YouTube"
  critérios/refs: AC-018 (O motor roda com runtime de JavaScript disponível)
  arquivos permitidos (e seus testes): docker/Dockerfile.extractor, extractor/server.js
  mensagem de commit: "T-019 versionamento-yt-dlp: Ativar um runtime de JavaScript para a extração do YouTube"
...
- NÃO edite tasks.md, NÃO rode onp-spec verify/audit e NÃO toque em outras tarefas — o orquestrador cuida disso.
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium &&
    rodar_tarefa 'faixa-2' 'T-020' 'Você executa UMA tarefa da feature "versionamento-yt-dlp" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/versionamento-yt-dlp/spec.md, .spec/features/versionamento-yt-dlp/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-020 — "Fazer os avisos do yt-dlp chegarem ao registro do servidor"
  critérios/refs: AC-019 (Avisos do motor chegam ao registro do servidor)
  arquivos permitidos (e seus testes): extractor/server.js
  mensagem de commit: "T-020 versionamento-yt-dlp: Fazer os avisos do yt-dlp chegarem ao registro do servidor"
...
- NÃO edite tasks.md, NÃO rode onp-spec verify/audit e NÃO toque em outras tarefas — o orquestrador cuida disso.
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium &&
    rodar_tarefa 'faixa-2' 'T-021' 'Você executa UMA tarefa da feature "versionamento-yt-dlp" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/versionamento-yt-dlp/spec.md, .spec/features/versionamento-yt-dlp/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-021 — "Permitir fixar e voltar a versão do yt-dlp"
  critérios/refs: AC-016 (Dá para fixar uma versão), AC-017 (A versão anterior fica guardada para o retorno)
  arquivos permitidos (e seus testes): docker/Dockerfile.extractor, docker-compose.yml, README-docker.md
  mensagem de commit: "T-021 versionamento-yt-dlp: Permitir fixar e voltar a versão do yt-dlp"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-2' 'spec/versionamento-yt-dlp-faixa-2' "$WT" "$st" || return 1
  marcar_concluidas T-017 T-018 T-019 T-020 T-021
  return 0
}
...
encerrar() { # $1=escopo
  ...
  if [ "$COM_GATE" -eq 0 ]; then
    ...
    if [ -z "$FALHAS" ]; then
      ...
      amarelo "  para o veredito: bash .spec/features/versionamento-yt-dlp/executar-tarefas.sh --gate"
      exit 0
    fi
...
listar() {
  ...
  echo "  faixa-1  onda 1  T-016"
  echo "  faixa-2  onda 1  T-017, T-018, T-019, T-020, T-021"
  echo
